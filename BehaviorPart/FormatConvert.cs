...
namespace BCatchAutoConverter
{
    internal static class FormatConvert
    {
        ...
        static void ConvertFormat(string ffmpegPath, string arg, string[] originPaths, bool isMerge)
        {
            ...
            TryDeleteFile(originPaths, isMerge);
        }
        static void ConvertFormat(string ffmpegPath, string arg, string originPath, bool isMerge)
            => ConvertFormat(ffmpegPath, arg, new string[1] { originPath }, isMerge);

        static void TryDeleteFile(string[] paths, bool isMerge)
        {
            bool b;
            if (isMerge)
                b = MainForm.Checked_DeleteAfterMerge;
            else
                b = MainForm.Checked_DeleteAfterFormatConvert;
        ...
        static void ConvertToMp3(string ffmpegPath, string inputFile, string outputFile)
        {
            ...
            ConvertFormat(ffmpegPath, arg, inputFile, false);
        }
        static void ConvertToMp4(string ffmpegPath, string inputFile, string outputFile)
        {
            ...
            ConvertFormat(ffmpegPath, arguments, inputFile, false);
        }
        public static string MergeToMp4()
        {
            ...
            try
            {
                ConvertFormat(FfmPath, arg, new string[2] { path_mp4, path_mp3 }, true);
                return null;
            }
            catch (Exception ex)
            {
